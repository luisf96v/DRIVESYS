...
const UserCtrl = {
    ...
    findAll: async (req, res) => {
        console.log(req.signedCookies.ouid)
        if(ObjectId.isValid(req.signedCookies.ouid)){
            User.find({ org: req.signedCookies.ouid })
    ...
    update: async (req, res) => {
        try {
            if((req.params.id && ObjectId.isValid(req.params.id)) || ObjectId.isValid(req.signedCookies.muid)){
                let user = req.body
